<template>
  <div class="page">
    ...
    <div class="grid lg:grid-cols-3 gap-6">
      <!-- Left col: existing locations list -->
      <div class="lg:col-span-2 space-y-3">
        ...
        <button @click="showAddForm = !showAddForm" class="btn-secondary w-full lg:hidden">
          {{ showAddForm ? '✕ Cancel' : '+ Add Location' }}
        </button>

        <!-- Mobile-only form -->
        <form v-if="showAddForm" @submit.prevent="addLocation" class="card space-y-3 lg:hidden">
      ...
      <!-- Right col: add location form (always visible on desktop, sticky) -->
      <div class="hidden lg:block lg:col-span-1 lg:sticky lg:top-24 lg:self-start">
        <form @submit.prevent="addLocation" class="card space-y-3">
          <h2 class="font-semibold text-sm text-gray-900">Add Delivery Location</h2>
          <div>
            <label class="label">Location Name *</label>
            <input v-model="newLoc.name" class="input" placeholder="Main Factory" required />
          </div>
